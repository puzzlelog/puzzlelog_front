  ...
  useEffect(() => {
    const fetchEmotionStickers = async () => {
      try {
        ...
        const emotionStickersData = response.data.data.filter(sticker => sticker.type === "emotion");
        setEmotionStickers(emotionStickersData);
      } catch (error) {
        console.error("감정 스티커 불러오기 실패:", error);
      }
    };
    ...
    const handleEmotionStickerSelected = async (emotionStickerId) => {
      ...
      setShowEmotionSelector(false); // 감정 패널 닫기
      ...
      if (canvasRef.current) {
        ...
        const diaryData = {
          ...
          themeColor: "#FFECCC",
          emotionContentId: emotionStickerId, // ✅ 여기 emotionContentId 저장
          isShared: false,
          openAt: null,
          elements
        };
    ...
    return (
      <div className="min-h-screen bg-[#F7F3E5] flex flex-col">
        ...
        <div className="flex flex-1 flex-col items-center">
          ...
          <button onClick={() => setShowEmotionSelector(true)} className="px-6 py-2 bg-[#D6B896] text-white rounded-lg shadow-md">
                          저장하기</button>
